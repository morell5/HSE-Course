...
{
    std::string out;
    if (this->negative_flag)
    {
        out += "-";
    }
    out += std::to_string(this->data.back());
    for (int i = this->data.size() - 2; i >= 0; i--)
    {
        out += setlen(BigInteger::SIZE, std::to_string(this->data[i]));
    }
    return out;
}
...
BigInteger::BigInteger(int number)
{
    ...
    else
    {
        while (number != 0)
        {
            this->data.push_back(number % BigInteger::BASE);
...
BigInteger::BigInteger(std::string number)
{
    ...
    else
    {
        ...
        for (int i = number.size(); i > 0; i -= BigInteger::SIZE)
        {
            if (i >= BigInteger::SIZE)
            {
                this->data.push_back(atoi(number.substr(i - BigInteger::SIZE, BigInteger::SIZE).c_str()));
            }
            else
            {
                this->data.push_back(atoi(number.substr(0, i).c_str()));
            }
        }
    }
    this->remove_zeros();
...
BigInteger &BigInteger::operator=(const BigInteger &other)
{
    ...
    this->negative_flag = other.negative_flag;
    return *this;
}
...
std::ostream &operator<<(std::ostream &out, const BigInteger &printing)
{
    ...
    for (int i = printing.data.size() - 2; i >= 0; i--)
    {
...
const BigInteger operator+(BigInteger a, const BigInteger &b)
{
    ...
    int carry = 0;
    for (int i = 0; i < std::max(a.data.size(), b.data.size()) || carry != 0; i++)
    {
        if (i == a.data.size())
        {
            a.data.push_back(0);
...
const BigInteger operator*(const BigInteger &a, const BigInteger &b) // деление при помощи алгоритма Карацубы
{
    if (a.negative_flag || b.negative_flag)
    {
        if (a.negative_flag ^ b.negative_flag)
    ...
    else
    {
        int k = n / 2;
        std::vector<int> a2;
        a2.insert(a2.begin(), a.data.begin(), a.data.begin() + std::min(k, (int)a.data.size()));
        std::vector<int> a1;
        if (k <= a.data.size())
        {
        ...
        std::vector<int> b2;
        b2.insert(b2.begin(), b.data.begin(), b.data.begin() + std::min(k, (int)b.data.size()));
        std::vector<int> b1;
        if (k <= b.data.size())
        {
...
const BigInteger operator/(const BigInteger &a, const BigInteger &b) // деление столбиком
{
    if (b == BigInteger(0))
    {
