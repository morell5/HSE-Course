...
const double side_square(const Point first, const Point second)
{
	double len = (first.x - second.x) * (first.x - second.x) + (first.y - second.y) * (first.y - second.y) / 2.0;
	return sqrt(len);
}
...
{
	double rad = lenght(this->first_point, this->second_point) / 2.0;
	Point center((this->first_point.x + this->second_point.x) / 2.0, (this->first_point.y + this->second_point.y) / 2.0);
	Circle result(rad, center);
	return result;
}
