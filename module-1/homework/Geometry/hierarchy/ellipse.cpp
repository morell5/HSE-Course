...
std::pair<Line, Line> Ellipse::directrices() {
    return std::make_pair(Line(Point(-a / e, 0), Point(-a / e, 1)),
        Line(Point(a / e, 0), Point(a / e, 1)));
}

double Ellipse::eccentricity() { return this->e; }

Point Ellipse::center() { return this->ellipse_center; }

double Ellipse::perimeter() { return 4 * (pi * a * b + (a - b)) / (a + b); }

double Ellipse::area() { return pi * (this->a * this->b); }

bool Ellipse::containsPoint(const Point& point) {
    double div_x = point.x * point.x / (this->a * this->a);
    double div_y = point.y * point.y / (this->b * this->b);
    if (div_x + div_y <= 1) {
        return true;
    }
    return false;
