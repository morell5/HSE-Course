...
Triangle::Triangle(Point& a, Point& b, Point& c) {
	...
	std::sort(v.begin(), v.end(), cmp);
	this->vertices.push_back(v[0]);
	if (v[0].y > v[1].y && v[1].y > v[2].y || v[0].y < v[1].y && v[1].y > v[2].y) {
		this->vertices.push_back(v[2]);
		this->vertices.push_back(v[1]);
	}
	else {
...
Circle Triangle::circumscribedCircle() {
	double sin_a = fabs(vec_pr(vec[0], vec[1])) / (vec[0].vector_len * vec[1].vector_len);
	double rad = this->vec[2].vector_len / sin_a / 2.0;
	Point p[4];
	for (int i = 0; i < 3; i++) {
		p[i + 1] = this->vertices[i];
...
double angle_side(Point& a, Point& b) {
...
Circle Triangle::inscribedCircle() {
	...
	double S = area_t(p[0], p[1], p[2]);
	double rad = S / (leng(p[0], p[1]) + leng(p[1], p[2]) + leng(p[2], p[0]));
	double x_c = (leng(p[0], p[1]) * p[2].x + leng(p[0], p[2]) * p[1].x + leng(p[1], p[2]) * p[0].x) / (leng(p[0], p[1]) + leng(p[0], p[2]) + leng(p[1], p[2]));
	double y_c = (leng(p[0], p[1]) * p[2].y + leng(p[0], p[2]) * p[1].y + leng(p[1], p[2]) * p[0].y) / (leng(p[0], p[1]) + leng(p[0], p[2]) + leng(p[1], p[2]));
	Circle res(rad, Point(x_c, y_c));
	return res;
