...
bool is_crossed(Point a, Point b, Point c, Point d) {
	return is_crossed_1(a.x, b.x, c.x, d.x)
		&& is_crossed_1(a.y, b.y, c.y, d.y)
...
bool Shape::containsPoint(Point point) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (is_crossed(point, point_to, this->vertices[i], this->vertices[next_i])) {
			counter++;
		}
		if (is_on_line(Line(point, point_to), this->vertices[i])) {
			int last_i = (i - 1 + n) % n;
			if (is_crossed(point, point_to, this->vertices[i], this->vertices[last_i])) {
				counter++;
			}
		}
	}
...
bool Shape::isSimilarTo(const Shape& another) {
	...
	for (int i = 0; i < n; i++) {
		int next_i = (i + 1) % n;
		int last_i = (i - 1 + n) % n;
		Vector v1(this->vertices[last_i].x - this->vertices[i].x, this->vertices[last_i].y - this->vertices[i].y, 0);
		Vector v2(this->vertices[next_i].x - this->vertices[i].x, this->vertices[next_i].y - this->vertices[i].y, 0);
		angle_this.push_back(std::fabs(v1 % v2 / (v1.len() * v2.len())));
	}
	for (int i = 0; i < n; i++) {
		int next_i = (i + 1) % n;
		int last_i = (i - 1 + n) % n;
		Vector v1(another.vertices[last_i].x - another.vertices[i].x, another.vertices[last_i].y - another.vertices[i].y, 0);
		Vector v2(another.vertices[next_i].x - another.vertices[i].x, another.vertices[next_i].y - another.vertices[i].y, 0);
	...
	for (int i = 0; i < n; i++) {
		...
		if (can) {
			for (int j = 0; j < n; j++) {
				int n_j = (j + 1) % n;
				int n_ji = (j + i + 1) % n;
				Vector v1(this->vertices[n_j].x - this->vertices[j].x, this->vertices[n_j].y - this->vertices[j].y, 0);
				Vector v2(another.vertices[n_ji].x - another.vertices[(j + i) % n].x, another.vertices[n_ji].y - another.vertices[(j + i) % n].y, 0);
				if (v1.len() != v2.len()) {
					return false;
				}
			}
...
void Shape::reflex(Line axis) {
	int n = this->vertices.size();
	Vector l(axis.first_point.x - axis.second_point.x, axis.first_point.y - axis.second_point.y, 0);
	for (int i = 0; i < n; i++) {
		Vector v(this->vertices[i].x - axis.second_point.x, this->vertices[i].y - axis.second_point.y, 0);
		Vector vv = l;
		double scl = ((v % l) / (l % l));
		vv = vv * 2;
		vv = vv * scl;
...
void Shape::rotate(Point center, double angle) {
	...
	int n = this->vertices.size();
	for (int i = 0; i < n; i++) {
		double xx = this->vertices[i].x, yy = this->vertices[i].y, x0 = center.x, y0 = center.y;
